...
hook.Add("HUDPaint", "ixCaptureHUD", function()
    ...
    if (isNeutral) then
        -- Live captures: neutrals are never locked. Use progress alone.
        if (progress >= 1 and capTeam ~= "" and capTeam ~= nil) then
            label = label .. "  -  " .. string.upper(capTeam) .. " HELD"
        elseif (contested) then
            label = label .. "  -  CONTESTED"
        elseif (capTeam == "" or capTeam == nil) then
            label = label .. "  -  NEUTRAL"
        end
    else
        -- Team-locked points: use existing locked-flag semantics.
