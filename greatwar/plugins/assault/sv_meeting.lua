...
-- ============================================================
--
-- Fires from the ix_capture_neutral_base Think when a player team
-- pushes a neutral point's progress to 1.0. We resolve the meeting
-- engagement on the FIRST capture — first to break through wins the
-- initiative. Subsequent neutral captures (if any complete around the
-- same tick) are ignored because meetingActive is already false.
...
hook.Add("NeutralCapturePointTaken", "ixAssaultMeetingResolve", function(point, capturingTeam)
    ...
    plugin:ResolveMeetingEngagement(capturingTeam)
end)
