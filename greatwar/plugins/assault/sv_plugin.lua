...
-- Reset progress/contested/capturing-team but DON'T touch locked state.
local function ResetCapturePointProgress(ent)
    ent:SetProgress(0)
    ent:SetCapturingTeam("")
    ent:SetContested(false)
end

-- Lock every capture point on the map and clear progress.
-- This is the "battle is dormant" state: nothing is capturable.
-- Used at map load, between assaults, and on win/fail.
local function LockAllCapturePoints()
    ForEachCapturePoint(function(ent)
        ResetCapturePointProgress(ent)
        ent:SetLocked(true)
    end)
end

-- Unlock every defender-side capture point in the given sector.
-- Used when an assault starts (sector A) or advances (sector B, etc.).
-- Returns count for diagnostics.
local function UnlockSectorForDefender(defenderTeam, sectorID)
    local n = 0
    ForEachCapturePoint(function(ent)
        if (ent:GetTeam() ~= defenderTeam) then return end
        if ((ent:GetSectorID() or "A") ~= sectorID) then return end
        ResetCapturePointProgress(ent)
        ent:SetLocked(false)
        n = n + 1
    end)
...
function PLUGIN:StartAssault(attackerTeam)
    ...
    -- ============================================================
    -- Readiness wiring (added Phase 1).
    -- Pull the Hasty/Standard flag stamped by the HQ vote-call gate.
    -- Default to "standard" defensively (e.g. meeting engagements may
    -- bypass the normal call flow).
    -- Then zero the attacker's readiness pool. Phase 3 will read
    -- ix.assault.strength to apply the actual mechanical differences.
    -- ============================================================
    local hqState = ix.hq and ix.hq.GetVoteState and ix.hq.GetVoteState(attackerTeam)
    ix.assault.strength = (hqState and hqState.assaultStrength) or "standard"

    if ix.readiness then
        -- force=true bypasses the lock that HQ set when the vote passed.
        -- The unlock itself happens in ix.hq.DisarmAssault, which fires
        -- right after StartAssault returns.
        ix.readiness.Set(attackerTeam, 0, "assault_started", true)
    end
    ...
    -- Unlock the first sector for the defender so attackers can begin capping.
    local opened = UnlockSectorForDefender(ix.assault.DEFENDER_TEAM, ix.assault.activeSector)

    BroadcastState()
    BroadcastEvent("started", ix.assault.activeSector, "", attackerTeam)
    print(string.format("[ASSAULT] Started. ATT=%s DEF=%s sector=%s (%d points open) strength=%s",
        attackerTeam, ix.assault.DEFENDER_TEAM, ix.assault.activeSector, opened,
        ix.assault.strength))
    return true
end
...
function PLUGIN:WinAssault()
    ...
    -- Lock everything down. Battle is over; no points are capturable until
    -- the next assault is ordered (or until the next map loads).
    LockAllCapturePoints()
...
function PLUGIN:FailAssault(reason)
    ...
    -- ============================================================
    -- Readiness wiring (added Phase 1).
    -- Phase 1 spec: defender holds the line -> +15 readiness; attacker
    -- forced to 0 (belt-and-suspenders since they were zeroed at start,
    -- but other plugins might have nudged them up since).
    -- Done BEFORE we wipe ATTACKER_TEAM / DEFENDER_TEAM below so the
    -- adjustments hit the right teams.
    -- ============================================================
    if ix.readiness then
        local attacker = ix.assault.ATTACKER_TEAM
        local defender = ix.assault.DEFENDER_TEAM
        if attacker then
            ix.readiness.Set(attacker, 0, "assault_failed", true)
        end
        if defender then
            local bonus = ix.readiness.DELTA and ix.readiness.DELTA.DEFENDER_HOLD_BONUS or 15
            ix.readiness.Add(defender, bonus, "defender_held", true)
        end
    end
    ...
    -- Defenders reclaim everything. Lock all points (no captures until the
    -- next dawn vote passes) and clear fallen-sector marks so the map is
    -- back to a fresh defender-side state.
    ix.assault.ResetFallen()
    LockAllCapturePoints()
    ReEnableAllForwardSpawns()
...
-- Advance to next sector after a clear. Adds time bonus AND unlocks
-- the next sector's capture points for the defender so attackers can
-- begin pushing into it.
local function AdvanceSector(prevSector)
    ...
    local opened = UnlockSectorForDefender(ix.assault.DEFENDER_TEAM, nextSector)

    BroadcastState()
    print(string.format("[ASSAULT] Sector advanced to %s (+%ds, %d points open)",
        nextSector, ix.assault.SECTOR_BONUS, opened))
end
