    ...
    function PANEL:GetTeamSpawns()
        ...
        for _, ent in ipairs(ents.GetAll()) do
            ...
            if isSpawnClass and ent:GetTeam() == team then
                -- Skip forward spawns whose sector has been overrun. Reserve
                -- spawns are never gated by the assault system.
                local skip = ent:GetSpawnType() == SPAWN_FORWARD
                    and ent.GetDisabled and ent:GetDisabled()
                if not skip then
