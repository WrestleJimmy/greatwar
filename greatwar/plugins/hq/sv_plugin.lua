...
local function SyncVoteToTeam(team)
    ...
    local payload = {
        ...
        -- Readiness fields (added Phase 1).
        readiness      = ix.readiness and ix.readiness.Get(team) or 0,
        readinessMin   = (ix.readiness and ix.readiness.MIN_THRESHOLD) or 30,
        readinessMax   = (ix.readiness and ix.readiness.MAX_THRESHOLD) or 100,
    }
...
function PLUGIN:OnHQUsed(client, hqTeam)
    ...
    local payload = {
        ...
        shortDateLabel = shortDateLabel,
        -- Readiness fields (added Phase 1).
        readiness      = ix.readiness and ix.readiness.Get(hqTeam) or 0,
        readinessMin   = (ix.readiness and ix.readiness.MIN_THRESHOLD) or 30,
        readinessMax   = (ix.readiness and ix.readiness.MAX_THRESHOLD) or 100,
    }
    ...
    -- Phase 2: ensure the readiness log on the client is fresh when the
    -- panel renders. SyncFullLogToPlayer self-gates on officer/fallback
    -- so non-eligible viewers get nothing — same policy as the log
    -- broadcast side. Cheap; runs once per menu open.
    if ix.readiness and ix.readiness.SyncFullLogToPlayer then
        ix.readiness.SyncFullLogToPlayer(client, hqTeam)
    end
end
...
net.Receive("ixHQVoteStart", function(len, client)
    ...
    -- ============================================================
    -- Readiness gate (added Phase 1).
    -- Refuse calls below MIN_THRESHOLD; tag the assault Hasty/Standard
    -- based on where the team is in the readiness band when the vote
    -- is called. The strength flag rides along on the vote state and
    -- is read by the assault plugin when StartAssault fires.
    -- ============================================================
    local readiness = ix.readiness and ix.readiness.Get(team) or 0
    local minThresh = (ix.readiness and ix.readiness.MIN_THRESHOLD) or 30
    local maxThresh = (ix.readiness and ix.readiness.MAX_THRESHOLD) or 100

    if readiness < minThresh then
        client:Notify(string.format(
            "Insufficient readiness. Need at least %d.", minThresh))
        return
    end

    state.assaultStrength = (readiness >= maxThresh) and "standard" or "hasty"
...
net.Receive("ixHQVoteCast", function(len, client)
    ...
    if yesCount >= threshold then
        ...
        -- Lock readiness at its current value (added Phase 1).
        -- From here until StartAssault fires (or the assault is disarmed),
        -- nothing can raise or lower this team's readiness. Unlock happens
        -- inside ix.hq.DisarmAssault (covers both success and stand-down
        -- paths) and inside StartAssault's force-zero step.
        if ix.readiness and ix.readiness.Lock then
            ix.readiness.Lock(team, "vote_passed")
        end
